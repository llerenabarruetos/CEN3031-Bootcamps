...
module.exports = function(req, res, next) {		//forward geocoding - address to coordinates
  if(req.body.address) { //if the request's body has an address
    ...
    var options = { 
      q: addressTemp4, // address to be geocoded = req.body.address (but formatted by the code above^)
      key: config.openCage.key,  
    }

    //Setup your request using URL and options - see ? for format
    request({
      url: 'https://api.opencagedata.com/geocode/v1/json',  //this url will be combined with q and key to be useful
      qs: options
